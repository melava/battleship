...
describe('test receive attack function', () => {
    ...
    test('attack an empty slot', () => {
        // console.log(myBoard.board)
        myBoard.receiveAttack(23, myBoard);
        expect(myBoard.board[23].occupied).toBe(null);
    ...
    test('attack several empty slots', () => {
        ...
        expect(myBoard.board[23].hit).toBeTruthy();
        expect(myBoard.missedHit).toEqual([23]);
        myBoard.receiveAttack(0, myBoard);
    ...
    test('hit the carrier in slot 54', () => {
        let shot = myBoard.receiveAttack(54, myBoard);
    ...
    test('hit and sink destroyer', () => {
        let shot1 = myBoard.receiveAttack(26, myBoard);
        let shot2 = myBoard.receiveAttack(36, myBoard);
...
describe('test receive attack function', () => {
    ...
    test('hit and sink all 3 ships on board', () => {
        let shot = myBoard.receiveAttack(26, myBoard);
        myFleet[shot].hit(26);
        expect(myBoard.allSunk).toBeFalsy();
        shot = myBoard.receiveAttack(36, myBoard);
        myFleet[shot].hit(36);
        expect(myBoard.allSunk).toBeFalsy();
        shot = myBoard.receiveAttack(71, myBoard);
        myFleet[shot].hit(71);
        expect(myBoard.allSunk).toBeFalsy();
        shot = myBoard.receiveAttack(2, myBoard);
        myFleet[shot].hit(2);
        expect(myBoard.allSunk).toBeFalsy();
        shot = myBoard.receiveAttack(3, myBoard);
        myFleet[shot].hit(3);
        expect(myBoard.allSunk).toBeFalsy();
        shot = myBoard.receiveAttack(4, myBoard);
        myFleet[shot].hit(4);
...
describe('initialize game and attack', () => {
    beforeEach(() => {
        myFirstPlayer = PlayerFactory('Penelope', GameBoardFactory(), fleetFactory());
        myFirstPlayer.board.chooseCoordinates(myFirstPlayer.fleet.submarine1, 71, 'h');
        mySecondPlayer = PlayerFactory('Cyril', GameBoardFactory(), fleetFactory());
    ...
    test('first player attack, hit then miss + second player attack and sink the submarine', () => {
        let attackCoordinates = 3;
        let shot = mySecondPlayer.board.receiveAttack(attackCoordinates, mySecondPlayer.board);
        if (shot){
            mySecondPlayer.fleet[shot].hit(attackCoordinates);
        }
        expect(mySecondPlayer.fleet[shot].hitArray).toEqual([3]);
        expect(mySecondPlayer.fleet[shot].sunk).toBeFalsy();
        expect(mySecondPlayer.board.allSunk).toBeFalsy();
        expect(mySecondPlayer.board.missedHit).toEqual([]);
        attackCoordinates = 13;
        shot = mySecondPlayer.board.receiveAttack(attackCoordinates, mySecondPlayer.board);
        if (shot){
            mySecondPlayer.fleet[shot].hit(attackCoordinates);
        }
        expect(mySecondPlayer.board.missedHit).toEqual([13]);
        expect(mySecondPlayer.board.allSunk).toBeFalsy();

        attackCoordinates = 71;
        shot = myFirstPlayer.board.receiveAttack(attackCoordinates, myFirstPlayer.board);
        if (shot){
            myFirstPlayer.fleet[shot].hit(attackCoordinates);
        }
        expect(myFirstPlayer.fleet[shot].hitArray).toEqual([71]);
        expect(myFirstPlayer.fleet[shot].sunk).toBeTruthy();
        expect(myFirstPlayer.board.missedHit).toEqual([]);
        expect(myFirstPlayer.board.allSunk).toBeTruthy();
        // console.log(myFirstPlayer.board)
        // console.log(mySecondPlayer.board)
    })

    test('try to make an illegal move -hit the same spot twice- and catch an error', () => {
        let attackCoordinates = 3;
        let shot = mySecondPlayer.board.receiveAttack(attackCoordinates, mySecondPlayer.board);
        if (shot){
            mySecondPlayer.fleet[shot].hit(attackCoordinates);
        ...
        attackCoordinates = 3;
        expect(() => mySecondPlayer.board.receiveAttack(attackCoordinates, mySecondPlayer.board)).toThrow();
    })
});